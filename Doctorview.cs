...
namespace Hospital
{
    public partial class Doctorview : Form
    {
        ...
        public Doctorview(decimal id, string fName, string lName, string gender, decimal phone, string email, string password, int salary, string speciality)
        {
            ...
            InitializeComponent();
            connectString = "Data Source =USER\\SQLEXPRESS;Initial Catalog=hospital;Integrated Security=true";
        ...
        private void populateItems()
        {
            ...
            while (rdr1.Read())
            {
                ...
                int aid = Convert.ToInt32(rdr1.GetInt32(4));
        ...
        private void DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridView1.Columns["delete"].Index && e.RowIndex >= 0)
            {
                ...
                SqlConnection con = new SqlConnection("Data Source=USER\\SQLEXPRESS;Initial Catalog=hospital;Integrated Security=True;TrustServerCertificate=True");
                con.Open();
        ...
        {
            if (comboBox1.SelectedIndex < 0 || comboBox1.SelectedIndex >= med.Count)
            {
                MessageBox.Show("Please select a valid medicine.", "Alert");
                return;
            }
            int mId = med[comboBox1.SelectedIndex];
            string dosage = textBox1.Text;
            string details = richTextBox1.Text;
            if (string.IsNullOrWhiteSpace(dosage) || string.IsNullOrWhiteSpace(details))
            {
